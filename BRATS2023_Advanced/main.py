...
from optimizers.lr_scheduler import StepLR
...
parser.add_argument("--optim_rho", default=0.9, type=float, help="running average of squared gradients")
parser.add_argument("--optim_eps", default=1e-6, type=float, help="improving numerical stability")
parser.add_argument("--optim_step_size", default=5, type=float, help="decaying the learning rate")
parser.add_argument("--optim_gamma", default=0.1, type=float, help="multiplicative factor of learning rate")
...
def main_worker(gpu, args):
    ...
    if args.optim_name == "adam":
        ...
        optimizer = torch.optim.Adadelta(model.parameters(), lr=args.optim_lr, rho=args.optim_rho, eps=args.optim_eps, weight_decay=args.reg_weight)
    else:
        raise ValueError("Unsupported Optimization Procedure: " + str(args.optim_name))

    if args.lrschedule == "step_lr":
        scheduler = StepLR(optimizer, step_size=args.optim_step_size, gamma=args.optim_gamma)
        if args.checkpoint is not None:
