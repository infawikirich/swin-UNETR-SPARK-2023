...
print("""
    The model was run for 120 epoch.
    The validation was done at 30 60 epoch.
    Optimizer - Adadelta lr = 1, rho = 0.9 eps = 1e-6, max_weight = 1e-5
    Scheduler - StepLR
    drop_rate = 0.0
    infer_overlap = 0.7
...
img_add = os.path.join(data_dir, "ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData/BraTS-GLI-00078-000/BraTS-GLI-00078-000-t2w.nii.gz")
label_add = os.path.join(data_dir, "ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData/BraTS-GLI-00078-000/BraTS-GLI-00078-000-seg.nii.gz")
...
model = SwinUNETR(
    ...
    out_channels=3,
    feature_size=60,
    drop_rate=0.5,     # changed from 0.0 to 0.7
    attn_drop_rate=0.0,    # changed from 0.0 to 0.0
    dropout_path_rate=0.0,     # changed from 0.0 to 0.0
    use_checkpoint=True,
).to(device)
...
dice_loss = DiceFocalLoss(include_background=True, to_onehot_y=False, sigmoid=True, gamma=2.0)
...
# Define new optimizers and scheduler using SGD with momentum and optimizers CosineAnnealingWarmRestarts
optimizer = torch.optim.Adadelta(model.parameters(), lr=1, rho=0.9, eps=1e-6, weight_decay=1e-5)
...
torch.save(model.state_dict(), "trained_swin_model_dicefocal_loss.pth")
